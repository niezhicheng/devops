...
<script lang="ts" setup>
  ...
  const currentRecord = ref<HostRecord | null>(null);

  const handleDelete = (record: HostRecord) => {
  ...
  const handleView = (record: HostRecord) => {
    // TODO: 实现查看详情功能
  ...
  const sftpFileList = ref<any[]>([]);
  const sftpSearchPath = ref('');
  const currentSftpHost = ref<HostRecord | null>(null);
  ...
  const handleSftp = (record: HostRecord) => {
  ...
  const handleSftpItemClick = (record: any) => {
    if (record.type === 'directory') {
      loadSftpFiles(record.path);
    }
  };

  const handleSftpUpload = async (option: any) => {
  ...
  const handleSftpDownload = async (record: any) => {
    if (!currentSftpHost.value) return;
  ...
  const handleSftpCompress = async (record: any) => {
    if (!currentSftpHost.value) return;
  ...
  const handleSftpRename = (record: any) => {
  ...
  const handleSftpFileDelete = async (record: any) => {
    if (!currentSftpHost.value) return;
  ...
  // 处理WebShell按钮点击
  const handleWebShell = (record: HostRecord) => {
    currentSftpHost.value = record;
